...
public class ValidationUtils {
    ...
    public static boolean isValidEmail(String email) {
        ...
        // Validasi dasar: harus ada @ dan .
        if (!email.contains("@") || !email.contains(".")) {
            return false;
        }

        // @ harus sebelum . (dot terakhir)
        int atIndex = email.indexOf("@");
        int dotIndex = email.lastIndexOf(".");

        // @ tidak boleh di awal, dot tidak boleh di akhir, @ harus sebelum dot
        if (atIndex == 0 || dotIndex == email.length() - 1 || atIndex >= dotIndex) {
            return false;
        }
    ...
    public static boolean isValidNomorTelepon(String telepon) {
        ...
        // Format valid:
        // - 08xxxxxxxxxx (lokal, 10–13 digit)
        // - +628xxxxxxxxxx (internasional, 11–14 digit)
        return teleponBersih.matches("^08[0-9]{8,11}$") ||
                teleponBersih.matches("^\\+628[0-9]{8,11}$");
    }
