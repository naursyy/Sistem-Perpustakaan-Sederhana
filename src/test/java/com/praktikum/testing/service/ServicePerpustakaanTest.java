...
public class ServicePerpustakaanTest {
    ...
    void setUp() {
        ...
        anggotaTest = new Anggota("A001", "John Student", "john@student.ac.id", "08123456789", Anggota.TipeAnggota.MAHASISWA);
    }
    ...
    void testHapusBukuGagalAdaYangDipinjam() {
        // Arrange - Ada yang dipinjam (5 total - 3 tersedia = 2 dipinjam)
        ...
        verify(mockRepositoryBuku).cariByIsbn("1234567890");
        verify(mockRepositoryBuku, never()).hapus(any(String.class));
    }
    ...
    void testPinjamBukuBerhasil() {
        ...
        when(mockRepositoryBuku.cariByIsbn("1234567890")).thenReturn(Optional.of(bukuTest));
        when(mockRepositoryBuku.updateJumlahTersedia("1234567890", 4)).thenReturn(true);
        ...
        verify(mockRepositoryBuku).cariByIsbn("1234567890");
        verify(mockRepositoryBuku).updateJumlahTersedia("1234567890", 4);
    }
    ...
    @Test
    @DisplayName("Hitung rating rata-rata berhasil")
    void testHitungRatingRataRataBerhasil() {
        bukuTest.tambahRating(4.0);
        bukuTest.tambahRating(5.0);

        when(mockRepositoryBuku.cariByIsbn("1234567890")).thenReturn(Optional.of(bukuTest));

        double rating = servicePerpustakaan.hitungRatingRataRata("1234567890");

        assertEquals(4.5, rating, 0.01);
        verify(mockRepositoryBuku).cariByIsbn("1234567890");
    }

    @Test
    @DisplayName("Hitung rating rata-rata buku tidak ada")
    void testHitungRatingRataRataBukuTidakAda() {
        when(mockRepositoryBuku.cariByIsbn("999")).thenReturn(Optional.empty());

        double rating = servicePerpustakaan.hitungRatingRataRata("999");

        assertEquals(0.0, rating);
        verify(mockRepositoryBuku).cariByIsbn("999");
    }

    @Test
    @DisplayName("Hitung rating rata-rata tanpa rating")
    void testHitungRatingRataRataTanpaRating() {
        when(mockRepositoryBuku.cariByIsbn("1234567890")).thenReturn(Optional.of(bukuTest));
